...
The agent is either not deployed or can't connect to the server. In case the agent is not deployed, please follow the deployment guide [here](../add_delete_node/add_delete_node.md).
...
* Page not found 404 - most likely caused by a 301 HTTP -> HTTPS redirect. Either change the SERVER\_ADDRESS on the server or the SERVER\_ADDRESS for the agent in the Start -> Team Cluster - Instructions -> Advanced settings
* ECONNREFUSED - check if you need to use an HTTP\_PROXY/HTTPS\_PROXY
