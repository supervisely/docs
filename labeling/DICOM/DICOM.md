...
**3D Perspective**: A 3D model that shows a volumetric rendering of the chest with the annotated area. It provides a clear 3D overview to quickly verify the shape and spatial position of your annotations.

Below the image are controls for toggling the visibility of different layers or components of the 3D view, such as annotations and slice planes.
...
- **Blue**: Coronal slice.
- **Red**: Axial slice.
- You can **toggle the visibility** of these slices by clicking the "eye" button, showing or hiding the corresponding **slice image plane** in the 3D view. This is useful when you want a cleaner 3D view or need to reduce rendering load.
...
The 3D perspective view is also optimized to run more smoothly and use fewer resources (especially on large volumes).
